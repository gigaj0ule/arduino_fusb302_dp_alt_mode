...
void StateMachineTypeC(Port_t *port)
{
    ...
    {
		// Read the interrupta, interruptb, status0, status1 and interrupt registers
...
#ifdef FSC_HAVE_SRC
                {
					// Otherwise we must be configured as a source or DRP
...
void StateMachineAttachedSource(Port_t *port)
{
    ...
    case 0:
        ...
        else if ((TimerExpired(&port->StateTimer) || port->PolicyHasContract) &&
                 (port->loopCounter != 0))
        {
...
void StateMachineAttachWaitAccessory(Port_t *port)
{
    ...
    if ((port->CCTermCCDebounce == CCTypeRa) 
            && (port->VCONNTerm == CCTypeRa)
            && port->PortConfig.audioAccSupport)
	{
        SetStateAudioAccessory(port);
    }
    else if (port->CCTerm == CCTypeOpen)
    {
        SetStateUnattached(port);
    }
...
void SetStateAttachWaitSink(Port_t *port)
{
    ...
    /* Check for detach before continuing */
...
void SetStateAttachWaitSource(Port_t *port)
{
    ...
    /* To help prevent detection of a non-compliant cable, briefly set the advertised
     * current to 3A here.  It will be reset after tAttachWaitAdv
     */
