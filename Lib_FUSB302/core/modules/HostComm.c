...
static FSC_U8 ReadReceivedSrcCaps(FSC_U8 *data, FSC_U32 bufLen, struct cPort *port)
{
    FSC_U32 i = 0, j = 0;
    FSC_U32 index = 0;
...
static FSC_U8 ReadReceivedSnkCaps(FSC_U8 *data, FSC_U32 bufLen, struct cPort *port)
{
    FSC_U32 i = 0, j = 0;
    FSC_U32 index = 0;
...
static void WriteSourceCapabilities(FSC_U8 *data, struct cPort *port, FSC_U32 len)
{
    FSC_U32 i = 0, j = 0;
...
#ifdef FSC_HAVE_SNK
static void WriteSinkCapabilities(FSC_U8 *data, struct cPort *port, FSC_U32 len)
{
    FSC_U32 i = 0, j = 0;
    sopMainHeader_t header = { 0 };
...
FSC_U8 ReadVdmModes(struct cPort *port, FSC_U8 *buf, FSC_U32 len)
{
...
void WriteVdmModes(struct cPort *port, FSC_U8 *buf)
{
    struct vdm_mode_buf *vdm_obj = (struct vdm_mode_buf*) buf;
...
static void SendUSBPDMessage(FSC_U8 *data, struct cPort *port, FSC_U32 len)
{
    FSC_U32 i = 0, j = 0;
    ...
    if (len < port->PDTransmitHeader.NumDataObjects * sizeof(FSC_U32)) { return; }
    /* Data objects */
...
static void ProcessTCSetState(ConnectionState state, struct cPort *port)
{
    switch (state)
    {
    case (Disabled):
...
static FSC_BOOL ProcessTCWrite(FSC_U8 *subCmds, struct cPort *port)
{
    ...
    for (i = 0; i < HCMD_PAYLOAD_SIZE / 2; i++)
    {
        ...
        case TYPEC_ACC_SUPPORT:
                ...
                pCmd->val != port->PortConfig.audioAccSupport)
            {
                ...
                port->PortConfig.audioAccSupport = port->PortConfig.poweredAccSupport;
                setUnattached = TRUE;
            }
            break;
        case TYPEC_SRC_PREF:
...
static FSC_BOOL ProcessTCRead(FSC_U8 *subCmds, FSC_U8 *outBuf, struct cPort *port)
{
    FSC_U32 len;
    FSC_U32 i;
...
static FSC_BOOL ProcessPDWrite(FSC_U8 *subCmds, struct cPort *port)
{
    ...
    for (i = 0; i < HCMD_PAYLOAD_SIZE / 2; i++)
    {
        ...
        case PD_OP_POWER:
            if (len < sizeof(FSC_U32)) { goto end_of_cmd; }
            port->PortConfig.SinkRequestOpPower = READ_WORD((&pCmd->val + 1));
            pCmd = (struct pd_buf*)((FSC_U8*)pCmd + pCmd->val);
            break;
        case PD_MAX_POWER:
            if (len < sizeof(FSC_U32)) { goto end_of_cmd; }
            port->PortConfig.SinkRequestMaxPower = READ_WORD((&pCmd->val + 1));
            pCmd = (struct pd_buf*)((FSC_U8*)pCmd + pCmd->val);
            break;
        case PD_HARD_RESET:
...
#ifdef FSC_HAVE_DP
static FSC_BOOL ProcessDPWrite(FSC_U8 *subCmds, struct cPort *port)
{
    ...
    for (i = 0; i < HCMD_PAYLOAD_SIZE / 2; i++)
    {
        ...
        case DP_AUTO_MODE_ENTRY:
            port->DisplayPortData.DpAutoModeEntryEnabled = (pCmd->val == 0) ? FALSE : TRUE;
            break;
        case DP_SEND_STATUS:
