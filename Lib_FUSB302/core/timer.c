...
FSC_BOOL TimerExpired(struct TimerObj *obj) {
  ...
  if (TimerDisabled(obj)) {
      ...
      /* Elapsed time >= period? */
      result = ((FSC_U32)(platform_get_system_time() - obj->starttime_) >= obj->period_)
              ? TRUE : FALSE;
  }
...
{
  FSC_U32 current_time = platform_get_system_time();
  ...
  if ((FSC_U32)(current_time - obj->starttime_) >= obj->period_) {
    return 1;
  }

  /* Timer hasn't expired, so this should return a valid time left. */
  return (FSC_U32)(obj->starttime_ + obj->period_ - current_time);
}
