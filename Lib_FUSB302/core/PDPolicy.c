...
void USBPDDisable(Port_t *port, FSC_BOOL DeviceUpdate)
{
    ...
    port->PdRevCable = PD_Specification_Revision;
    ...
    case 2:
        if (TimerExpired(&port->PolicyStateTimer))
        {
            ...
            if (Attempts_DiscvId_SOP_P_First)
            {
                ...
                } else if (port->PdRevContract == USBPDSPECREV2p0 &&
                           port->PolicyIsDFP) {
                    requestDiscoverIdentity(port, SOP_TYPE_SOP1);
                    port->discoverIdCounter++;
                }
...
void PolicySourceReady(Port_t *port)
{
    if (port->ProtocolMsgRx)
    {
        ...
        if (port->PolicyRxHeader.NumDataObjects == 0)
        {
            ...
            case CMTNotSupported:
                /* Received Not_Supported is ignored - notify DPM if needed */
                break;
            /* Unexpected messages */
            case CMTAccept:
            case CMTReject:
             ...
             (port->PartnerCaps.FPDOSink.DualRolePower == 1))
    {
        SetPEState(port, peSourceSendPRSwap);
             ...
             port->PartnerCaps.FPDOSink.DataRoleSwap)
    {
...
void PolicySinkEvaluateCaps(Port_t *port)
{
    ...
    for (i = 0; i < port->SrcCapsHeaderReceived.NumDataObjects; i++)
    {
        ...
        {
        case pdoTypeFixed:
            objVoltage = port->SrcCapsReceived[i].FPDOSupply.Voltage;
            ...
            else
            {
                ...
                objCurrent = port->SrcCapsReceived[i].FPDOSupply.MaxCurrent;
...
void PolicySinkReady(Port_t *port)
{
             ...
             (port->PartnerCaps.FPDOSupply.DualRolePower == TRUE))
    {
        SetPEState(port, peSinkSendPRSwap);
             ...
             port->PartnerCaps.FPDOSupply.DataRoleSwap)
    {
    ...
    case CBL_RST_START:
        if (port->PolicyIsDFP)
        {
            ...
            else
            {
                ...
                port->cblRstState = CBL_RST_WAIT_VCONN_SWAP;
            }
        }
        else if (port->IsVCONNSource)
        {
            if (port->PolicyIsDFP)
            {
                SetPEState(port, peSendCableReset);
            ...
            else
            {
                ...
                port->cblRstState = CBL_RST_WAIT_DR_SWAP;
            }
        }
        break;
    case CBL_RST_WAIT_VCONN_SWAP:
        if (port->IsVCONNSource)
        {
            SetPEState(port, peSendCableReset);
        }
